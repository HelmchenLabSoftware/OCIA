...
try
    ...
    % then save
    if exist(g_tWT.DefaultSavePath) & ~isempty(sDefaultBackupPath)
...
% Change window title
sWinTitle = get(findobj('Tag', 'WTMainWindow'), 'Name');
nIndx = findstr(sWinTitle, '-'); % TODO: FIX THIS, LINES ARE USED ALSO IN FILENAMES!!!!
sWinTitle = [sWinTitle(1:nIndx) ' ' g_tWT.DefaultSavePath];
set(findobj('Tag', 'WTMainWindow'), 'Name', sWinTitle);
