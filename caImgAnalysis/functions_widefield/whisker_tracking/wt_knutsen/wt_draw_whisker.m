...
if ~isempty(findobj('Tag', sprintf('whisker%d', w)))
    ...
    hSpline = plot(vXX, vYY, ':', 'LineWidth', g_tWT.WhiskerWidth, ...
        ...
        'Tag', sprintf('whisker%d', w));
    hScatHand = plot(vX, vY, 'k.', 'LineWidth', 10, 'Tag', sprintf('scatpt%d', w));
    
    if ~strcmp(sOption, 'nomenu')
        % Set spline properties
        hSplineMenu(w) = uicontextmenu;
        set(hSpline, 'Tag', sprintf('whisker%d', w), 'uicontextmenu',  hSplineMenu(w))
        uimenu(hSplineMenu(w), 'Label', 'Track manual without deletion', 'Callback', [sprintf('wt_track_manual(%d, ''without_deletion'')', w)])
        uimenu(hSplineMenu(w), 'Label', 'Track manual with deletion', 'Callback', [sprintf('wt_track_manual(%d)', w)])
        uimenu(hSplineMenu(w), 'Label', 'Clear from current frame...', 'Callback', [sprintf('global g_tWT;nLast=str2num(char(inputdlg(''Delete until frame'',''WT'')));if isempty(nLast), return, end;wt_clear_whisker(%d, get(g_tWT.Handles.hSlider,''Value''):nLast)', w)])
        
        uimenu(hSplineMenu(w), 'Label', 'Add point', 'Callback', [sprintf('wt_mark_whisker(''addpoint'', %d)', w)], 'Separator', 'on')
        uimenu(hSplineMenu(w), 'Label', 'Remove point', 'Callback', [sprintf('wt_mark_whisker(''removepoint'', %d)', w)])
        uimenu(hSplineMenu(w), 'Label', 'Constrain mid-point', 'Callback', [sprintf('wt_constrain_midpoint(%d)', w)])
        uimenu(hSplineMenu(w), 'Label', 'Move base-point', 'Callback', [sprintf('wt_mark_whisker(''movebasepoint'',%d)', w)])
        
        uimenu(hSplineMenu(w), 'Label', 'Set last frame', 'Callback', [sprintf('wt_mark_whisker(''setlastframe'', %d)', w)], 'Separator', 'on')
        uimenu(hSplineMenu(w), 'Label', 'Add touch location', 'Callback', sprintf('wt_mark_object(%d,''add'')', w));
        uimenu(hSplineMenu(w), 'Label', 'Set full length', 'Callback', sprintf('wt_mark_whisker(''setfulllength'', %d)', w));
        uimenu(hSplineMenu(w), 'Label', 'Set whisker identity', 'Callback', sprintf('wt_set_identity(%d,''whisker'')', w));
        uimenu(hSplineMenu(w), 'Label', 'Mark whisker label', 'Callback', sprintf('wt_track_whisker_label(%d, ''mark'')', w));
        
        uimenu(hSplineMenu(w), 'Label', 'Clean', 'Callback', [sprintf('wt_clean_splines(%d)', w)], 'Separator', 'on')
        uimenu(hSplineMenu(w), 'Label', 'Delete', 'Callback', [sprintf('wt_clear_whisker(%d)', w)])
        uimenu(hSplineMenu(w), 'Label', 'Copy', 'Callback', [sprintf('wt_copy_paste_whisker(''copy'', %d)', w)])
    end
end
