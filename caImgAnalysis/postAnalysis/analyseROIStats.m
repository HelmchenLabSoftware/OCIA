function ROIStats = analyseROIStats(config)
...
o('        #analys..ngleDay: %d ROIs, %d runs, %d stim. types.', nROIs, nRuns, numel(stimIDs), 2, dbgLevel);
...
if isfield(config, 'sgfilter') && ~isempty(config.sgfilter);
    filtTic = tic; % for performance timing purposes
...
if config.doPSAnalysis;
    ...
    for iFilt = 1 : numel(filters);
        ...
        ROIStats.PS.(filters{iFilt}).raw = extractPSTrace(ROIStats.runs.(filters{iFilt}), config.stim, config.PSFrames);
    end;
    o('        #analys..ngleDay: peri-stimulus trace extraction done (%3.1f sec).', toc(PSExtractTic), 2, dbgLevel);
    ...
    nPSFrames = size(ROIStats.PS.unfilt.raw{1, 1}, 2);
    nStims = size(ROIStats.PS.unfilt.raw, 2);
    nPSEvokedFrames = nPSFrames - config.PSFrames.base;
    EvokedRange = config.PSFrames.base + 1 : nPSFrames;
    PSTime = ((1 : nPSFrames) - config.PSFrames.base) / config.frameRate;
    % PSTimeEvoked = (EvokedRange - config.PSFrames.base) / config.frameRate;
    o('        #analys..ngleDay: %d stim(s), %d PSFrame(s).', nStims, nPSFrames, 2, dbgLevel);

    % normalize the peri-stimulus traces by substracting the mean of the baseline
    for iFilt = 1 : numel(filters);
        ROIStats.PS.(filters{iFilt}).norm = cell(size(ROIStats.PS.(filters{iFilt}).raw));
        for iStim = 1 : nStims;
            for iROI = 1 : nROIs;
                nTrials = size(ROIStats.PS.unfilt.raw{iROI, iStim}, 1);
                ROIStats.PS.(filters{iFilt}).norm{iROI, iStim} = zeros(nTrials, nPSFrames);
                for iTrial = 1 : nTrials;
                    rawTrace = ROIStats.PS.(filters{iFilt}).raw{iROI, iStim}(iTrial, :);
                    ROIStats.PS.(filters{iFilt}).norm{iROI, iStim}(iTrial, :) = rawTrace - ...
                            mean(rawTrace(1 : config.PSFrames.base));
                end;
            end;
        end;
    end;
    o('        #analys..ngleDay: peri-stimulus traces normalized.', 2, dbgLevel);

    % average across the "trials" (repetitions)
    PSStatCalcTic = tic; % for performance timing purposes
    % stats = {'mean', 'std', 'sem'};
    % stats = {'mean', 'std'};
    % stats = {};
    stats = config.PSAnalysisStats;
    % statsSecondaryArgs = {{2}, {[], 2}, {2}};
    % dims = {'trial', 'ROI', 'stim'};
    dims = {'trial'};
            ...
            for iStat = 1 : numel(stats);
                ...
                switch(dim);
                    case 'trial';
                        ...
                        % reshape to a 3D matrix of sizenPSFrames x nROIs x nStims
                        ...
                        ROIStats.PS.(filt).(dim).(stat).allNorm = ROIStats.PS.(filt).(dim).(stat).all - ...
                            repmat(nanmean(ROIStats.PS.(filt).(dim).(stat).all(1 : config.PSFrames.base, :, :)), nPSFrames, 1);
...
%     o('  #analyseROIStatsSingleDay: no event detection.', 4, dbgLevel);
% else
%     o('  #analyseROIStatsSingleDay: event detection method: "%s".', config.eventDetect.method, 4, dbgLevel);
%     
%     eventDetector(ROIStats, config.stim, ROISet, config.eventDetect.method, config.frameRate, ...
%         config.bpfilter, config.PSFrames, 'eventDetectTEst');
%     
% end;
...
%         runBoundaries;                  % run boundary labels
    ...
    if config.doSaveROICaTracesHeatMapAllRunsPlot;
        figHeatMap = plotROICaTracesHeatMap(commonArgs{:});
...
if config.doSaveROICaTracesPlot || config.doSaveROICaTracesHeatMapPlot;
    ...
    for iRun = 1 : nRuns;
        ...
        caTraces = cell2mat(ROIStats.runs.(filters{1})(:, iRun));
...
% stats2 = {'mean', 'max', 'median', 'sum', 'max3pp', 'respProb', 'tStatMax3PP', 'tStatMax', 'tStatMean', 'GLM'};
% stats2 = {'mean', 'max', 'median', 'sum', 'respProb', 'ttest', 'GLM'};
% stats2 = {'mean', 'max', 'median', 'sum', 'respProb', 'GLM'};
% stats2 = {'sum', 'respProb', 'GLM'};
for iFilt = 2 : numel(filters);
    filt = filters{iFilt};
...
if config.doSavePSAvgPlot;
    ...
    for iFilt = 2 : numel(filters);
        ...
        if ismatrix(ROIStats.PS.(filt).trial.mean.all);
...
%% PSPlot with average and stimulus tick and box
if config.doSavePSSingleAvg;
    ...
    for iFilt = 2 : numel(filters);
        ...
        figHandsPSSingleAvg = plotPSSingleAvg(ROIStats.PS.(filt).raw, PSTime, ROISet, stimIDs, saveNameStim, 5, 1, ...
            config.stimTime, [config.plotLimits(1) config.plotLimits(2)], 1);
        for iFig = 1 : numel(figHandsPSSingleAvg);
            saveFigToDir(figHandsPSSingleAvg(iFig), sprintf('%s_%02d', saveNameStim, iFig), 'PSAvgSinglePlot', ...
...
if config.doSaveInterTrialAvg;
    ...
    for iFilt = 2 : numel(filters);
        ...
        figHandsInterTrialAadaption = plotTrialAdaptionAvg(caTracesFilt, config.stim{1,1}, config.stimIDs, ROISet, 5, ...
            config.numOfStimPerBlock, config.stimDuration, config.frameRate, saveNameStim);
        for iFig = 1 : numel(figHandsInterTrialAadaption);
            saveFigToDir(figHandsInterTrialAadaption(iFig), sprintf('%s_%02d', saveNameStim, iFig), 'TrialAdaptionAvg', ...
...
if config.doSavePSPlotAllStimHeatMap;
    ...
    for iFilt = 2 : numel(filters);
        ...
        % plot the ROIStats as heat map
        figAllStimsAllROI = plotPSAllStimROIHeatMapPlot(saveNameAllStimsAllROI, ROIStats.PS.(filt).trial.mean.all, ...
...
if config.doSaveROITuningMaps   
    ...
    for iFilt = 2:numel(filters)
        ...
        if isfield(ROIStats.stats.(filt), 'ROIStimPreference')
            plotROITuningMaps(dimX, dimY, config.refImgPath, config.stimIDs, ROISet, ROIStats.stats.(filt).ROIStimPreference, config.saveName, config.doSaveROITuningMaps);
        else
...
if config.doSaveMaps;
    ...
    for iFilt = 2 : numel(filters);
        ...
        figEvokedPeak = plotROILocHeatMapPoint(maxRespAtBS, peakRespLims, refImg, ...
            dimX, dimY, ROILocs, saveNameMapPeakEvoked, 'Peak Evoked Resp. at BS [?]');
        tightfig();
        ...
        figEvokedMean = plotROILocHeatMapPoint(meanResp, meanRespLims, refImg, ...
            dimX, dimY, ROILocs, saveNameMapMeanEvoked, 'Mean Evoked Resp. [?]');
        tightfig();
