...
sessDirs = dir();
sessDirs(arrayfun(@(i) isempty(regexp(sessDirs(i).name, '^exp\d\d_\w+$', 'once')), 1 : numel(sessDirs))) = [];
...
nFrames = 80; frameRate = 20; baseLineTime = 0.5;
...
framesToAvgCell = { 15 : 20 };
timePeriodCell = { 'x0p25To0p50Sec' };
...
for iPer = 1 : numel(timePeriodCell);
    ...
    % create figure
    figHs(iPer) = figure('NumberTitle', 'off', 'Name', timePeriod(2:end), 'Position', pos, 'Color', 'white');
    ...
    for iSess = 1 : numel(sessDirs);
        ...
        if ~isfield(saveStruct(iSess), timePeriod) || isempty(saveStruct(iSess).(timePeriod));
            avgMat = load([sessDirs(iSess).name, '/Matt_files/stim_ave.mat']);
        ...
        sessNameClean = regexprep(regexprep(sessDirs(iSess).name, 'exp\d+_(\w+)', '$1'), '_', ' ');
        sessNum = regexprep(sessDirs(iSess).name, 'exp(\d+)_\w+', '$1');
...
%     export_fig(sprintf('analysis/%s.png', timePeriod(2:end)), '-r300', gcf);
%     export_fig(sprintf('analysis/%s.fig', timePeriod(2:end)), gcf);
