% runOCIABatchSave_gc3 - [no description]
%
%       runOCIABatchSave_gc3()
%
% [No description]
%
...
% diary(sprintf('/home/gc3-user/HIFONASHE-02/Analysis/Balazs_Laurenczy/2015/1502_chronic/A/%s_batchSave.log', datestr(now, 'yyyymmdd_HHMMSS')));
...
dayIDs = [
    ...
    arrayfun(@(day) sprintf('2015_04_%02d', day), [2, 16, 24, 27], 'UniformOutput', false) ...
...
%     struct( 'hifo', struct('animals', 1 : numel(animalIDs), 'days',  1 : numel(dayIDs), 'spots', 1 : numel(spotIDs)) ), ...
    struct( 'hifo', struct('animals', 1, 'days',  1 : numel(dayIDs), 'spots', 1 : numel(spotIDs)) ), ...
};
        ...
        for iDay = 1 : nDays;
            ...
            for iSpot = 1 : nSpots;
                ...
                try
                    ...
                    this = OCIA('batchSave_gc3', { animalID, dayID, spotID, '', 'ROISet ~= \w+' });
