% runOCIABatchAnalysis_OoPC - [no description]
%
%       runOCIABatchAnalysis_OoPC()
%
% [No description]
%
...
diary(sprintf('D:/Users/BaL/PhD/Analysis/1502_chronic/%s_batchAnalysis.log', datestr(now, 'yyyymmdd_HHMMSS')));
...
% create the tasks
tasks = { ...
%     struct( 'OoPC', struct('animals', 1 : numel(animalIDs), 'spots', 1 : numel(spotIDs)) ) ...
    struct( 'OoPC', struct('animals', 2, 'spots', 1 : numel(spotIDs)) ) ...
};
...
for iTask = 1 : nTasks;
    ...
    for iAnimal = 1 : nAnimals;
        ...
        for iSpot = 1 : nSpots;
            ...
            try
                this = OCIA('batchAnalysis_OoPC', { animalID, '', spotID, '', '' });
...
o('#%s: done (%.0f seconds).', mfilename(), toc(allTic), 0, 0);
