function this = OCIA_config_gregorRCaMP(this)
% OCIA_config_gregorRCaMP - "gregorRCaMP" OCIA configuration file
%
%       this = OCIA('gregorRCaMP')
%
% Configuration file for OCIA using the "gregorRCaMP" configuration. This function should not be called directly
%   but rather using the "this = OCIA('gregorRCaMP');" syntax to launch the software with this configuration.
%
...
this.path.rawData = 'W:\Neurophysiology\RawData\Gregor_and_Stefano\';
...
this.dw.annotateTableFunctionName = 'gregor';
...
...     id              label               parent    	visible     tooltip                     pattern
        ...
        'spot',         'Spot',             'day',      true,       'Spot folder',                      '^(?<spot>[Ss]pot\d+)$';
        'img',          'Imaging folder',   'spot',     true,       'Imaging data folder', ...
            '^(?<day>\d{4}_\d{2}_\d{2})__(?<time>\d{2}_\d{2}_\d{2})h$';
...
... id                          label                                                       description
    ...
    'ROIProp_distr',            'ROI property distribution',                                'Distribution of a property of the ROIs (response, SI, ...)';
...
this.an.img.ROIProp = 'responsiveness';
...
this.an.img.ROIPropPlotType = 'distribution';
...
% F0/R0 method index. Methods are mean, median, prctile, polyfit,
% polyfitMean, slidingMean
%this.an.img.f0method = 3;
this.an.img.f0method = 6;
% F0/R0 parameter (can be empty), for percentile: Nth percentile; for polyfit: polyfit degree, for slidingMean: windowsize (sec)
