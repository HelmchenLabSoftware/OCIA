...
%         sessIDs = dir([basePathAnim 'exp*']);
        ...
        for iSess = 1 : numel(sessIDs);
            ...
            if exist(tifFilesPath, 'dir') ~= 7;
                fprintf('      Skipping animal %d %s, day %s, session %d %s: Tiff_files folder missing.\n', ...
            ...
            imgForSize = imread(sprintf('%sTrial1frame2', tifFilesPath));
            imgDimY = size(imgForSize, 1);
            imgDimYDS = ceil(imgDimY / ds);
            imgDimX = size(imgForSize, 2);
                    ...
                    % trial not done yet
                        ...
                        parfor iFrame = 1 : nFrames;
                            ...
                            try
                                ...
                                fprintf('/!\\ Problem with animal %d %s, day %s, session %d %s trial %d frame %d (%s): %s. Skipping...\n', ...
                                    iAnim, animID, dayID, iSess, sessIDs(iSess).name, iTrial, iFrame, ...
                                    err.identifier, err.message); %#ok<PFBNS>
                                continue;
                            end;
