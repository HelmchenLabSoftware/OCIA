cd /fat2/Ariel_Gilad/Matlab_analysis/experiments/vsdi_coVnon_path/Path/legolas/2007_04_18/no_stim
...
for i=[4 3 2 6]%condition count
    ...
    eval(['b=size(cond',int2str(i),'n_dt_bl,2)-100;'])
    for p=1:size(roi_V1,1)
        for h=1:a  %trial count 
            ...
            l=0;
            disp(['condition #',int2str(i),'trial #',int2str(h),' calculating crossence'])
            ...
            %eval(['cross_V1_cond',int2str(i),'_t=t(:,:,ones(1,size(pixels,1))).*conj(f)./sqrt(abs(t(:,:,ones(1,size(pixels,1)))).^2.*abs(f).^2);'])
            if z==1
                eval(['cross_V1_cond',int2str(i),'=cross_V1_cond',int2str(i),'_t;'])
                %eval(['cross_V1_std_cond',int2str(i),'=cross_V1_cond',int2str(i),'_t.^2;'])
            else
                eval(['cross_V1_cond',int2str(i),'=cross_V1_cond',int2str(i),'+cross_V1_cond',int2str(i),'_t;'])
                %eval(['cross_V1_std_cond',int2str(i),'=cross_V1_std_cond',int2str(i),'+cross_V1_cond',int2str(i),'_t.^2;'])
            end
            eval(['clear cross_V1_cond',int2str(i),'_t'])
        end;
    eval(['cross_V1_cond',int2str(i),'=cross_V1_cond',int2str(i),'/z;'])
    %eval(['cross_V1_std_cond',int2str(i),'=sqrt((cross_V1_std_cond',int2str(i),'-(cross_V1_cond',int2str(i),'.^2)*z)/(z-1));'])
    eval(['save cross_V1_cond',int2str(i),'_pix_',int2str(roi_V1(p)),' cross_V1_cond',int2str(i),' roi_V1 pixels'])
    %eval(['save cross_V1_std_cond',int2str(i),'_pix_',int2str(roi_V1(p)),' cross_V1_std_cond',int2str(i),' roi_V1 pixels'])
    eval(['clear cross_V1_cond',int2str(i)])
    %eval(['clear cross_V1_std_cond',int2str(i)])
    z=0;
    end;
