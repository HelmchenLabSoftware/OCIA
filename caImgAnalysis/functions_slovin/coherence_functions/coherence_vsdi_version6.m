cd /fat2/Ariel_Gilad/Matlab_analysis/experiments/test/1005b/way3
...
for i=[4 3 2 6]%condition count
    ...
    q=q+1;
    if q==1
       eval(['coher_V1_cond',int2str(i),'=abs(e).^2./(v.*repmat(v(roi_V1(p),:,:),[10000 1 1]));'])
    else
       eval(['coher_V1_cond',int2str(i),'=coher_V1_cond',int2str(i),'+abs(e).^2./(v.*repmat(v(roi_V1(p),:,:),[10000 1 1]));'])
    end
    clear e
    clear v
    end;
