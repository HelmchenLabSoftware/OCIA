function [spect_ave,baseline_ave,baseline_std]=spectogram_for_data_EEG_way_der(data,win,baseline,roi)
...
spect=zeros(NFFT/2,total-win);             %when returning to the original data use size(data,2)
for h=1:total:size(PSD_EEG_ROI,2)      %when returning to the original data use size(data,2)
    k=k+1;
    %disp(h)
    for j=1:(total-win)      %when returning to the original data use size(data,2)
        %w=hamming(win).';
        w=hanning(win).';
