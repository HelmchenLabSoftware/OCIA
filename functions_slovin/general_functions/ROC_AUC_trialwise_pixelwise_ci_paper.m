...
for i=[7 26 29]
    figure
    mimg(mfilt2(cond4n_dt_bl(:,45:55,i)-1,100,100,1,'lm'),100,100,0e-3,1.5e-3);colormap(mapgeog)
end

for i=[7 26 29]
    figure
    mimg(mfilt2(cond5n_dt_bl(:,43:53,i)-1,100,100,1,'lm'),100,100,0e-3,1.5e-3);colormap(mapgeog)
end

for i=20
    figure
    mimg(mfilt2(mean(cond4n_dt_bl(:,43:53,i),2)-1,100,100,1,'lm'),100,100,0e-3,1.5e-3);colormap(mapgeog)
end

for i=1
    figure
    mimg(mfilt2(mean(cond1n_dt_bl(:,48:58,i),2)-1,100,100,1,'lm'),100,100,.5e-3,2.5e-3);colormap(mapgeog)
end

%1111c cond1
y=mfilt2(mean(cond1n_dt_bl(:,50:51,14),2)-1,100,100,1.5,'lm');
y(pixels_to_remove)=100;
figure;mimg(y,100,100,.4e-3,1.6e-3);colormap(mapgeog)
hold on
h=zeros(10000,1);
h(roi_contour2)=1;
...
[n1 x1]=hist(mean(cond1n_dt_bl(roi_contour2,50:51,14),2)-1,-1.5e-3:1.5e-4:3e-3);
[n2 x2]=hist(mean(cond1n_dt_bl(roi_maskin,50:51,14),2)-1,-1.5e-3:1.5e-4:3e-3);
figure;bar(x1,[n1/size(roi_contour2,1);n2/size(roi_maskin,1)]')
xlim([-1e-3 3e-3]);
...
ranksum(mean(cond1n_dt_bl(roi_contour2,50:51,14),2)-1,mean(cond1n_dt_bl(roi_maskin,50:51,14),2)-1)


%1111d cond2    
y=mfilt2(mean(cond2n_dt_bl(:,49:52,20),2)-1,100,100,1,'lm');
y(pixels_to_remove)=100;
figure;mimg(y,100,100,-.2e-3,1.2e-3);colormap(mapgeog)
hold on
h=zeros(10000,1);
h(roi_contour2)=1;
contour(reshape(h,100,100)')
...
text(100,35,'V2','FontSize',16)
text(96,22,'V4','FontSize',16)
[n1 x1]=hist(mean(cond2n_dt_bl(roi_contour2,49:52,20),2)-1,-1.5e-3:1.5e-4:3e-3);
[n2 x2]=hist(mean(cond2n_dt_bl(roi_maskin,49:52,20),2)-1,-1.5e-3:1.5e-4:3e-3);
figure;bar(x1,[n1/size(roi_contour2,1);n2/size(roi_maskin,1)]')
xlim([-1e-3 3e-3]);
legend('circle','background')
scores=[mean(cond2n_dt_bl(roi_contour2,49:52,20),2)-1;mean(cond2n_dt_bl(roi_maskin,49:52,20),2)-1]';
labels=ones(1,size(scores,2));
labels(size(mean(cond2n_dt_bl(roi_contour2,49:52,20),2)-1,1)+1:end)=0;
[X_tr20,Y_tr20,THRE,AUC_tr20,OPTROCPT,SUBY,SUBYNAMES] = perfcurve(labels,scores,1);
figure;plot(X_tr20,Y_tr20)
...
text(100,35,'V2','FontSize',16)
text(96,22,'V4','FontSize',16)
[n1 x1]=hist(mean(cond2n_dt_bl(roi_contour2,48:50,11),2)-1,-1.5e-3:1.5e-4:2.5e-3);
[n2 x2]=hist(mean(cond2n_dt_bl(roi_maskin,48:50,11),2)-1,-1.5e-3:1.5e-4:2.5e-3);
figure;bar(x1,[n1/size(roi_contour2,1);n2/size(roi_maskin,1)]')
legend('circle','background')
...
ranksum(mean(cond2n_dt_bl(roi_contour2,48:50,11),2)-1,mean(cond2n_dt_bl(roi_maskin,48:50,11),2)-1)

 
%1111c cond4
y=mfilt2(mean(cond4n_dt_bl(:,43:53,11),2)-1,100,100,1,'lm');
y(pixels_to_remove)=100;
figure;mimg(y,100,100,.4e-3,1.6e-3);colormap(mapgeog)
hold on
h=zeros(10000,1);
...
text(100,35,'V2','FontSize',16)
text(96,22,'V4','FontSize',16)
[n1 x1]=hist(mean(cond4n_dt_bl(roi_contour2,43:53,11),2)-1,-1.5e-3:1.5e-4:3e-3);
[n2 x2]=hist(mean(cond4n_dt_bl(roi_maskin,43:53,11),2)-1,-1.5e-3:1.5e-4:3e-3);
figure;bar(x1,[n1/size(roi_contour2,1);n2/size(roi_maskin,1)]')
xlim([-1e-3 3e-3]);
legend('circle','background')
scores=[mean(cond4n_dt_bl(roi_contour2,43:53,11),2)-1;mean(cond4n_dt_bl(roi_maskin,43:53,11),2)-1]';
labels=ones(1,size(scores,2));
labels(size(mean(cond4n_dt_bl(roi_contour2,43:53,11),2)-1,1)+1:end)=0;
[Xn_tr11,Yn_tr11,THRE,AUCn_tr11,OPTROCPT,SUBY,SUBYNAMES] = perfcurve(labels,scores,1);
figure;plot(Xn_tr11,Yn_tr11)


%1111c cond5
y=mfilt2(mean(cond5n_dt_bl(:,43:53,16),2)-1,100,100,1.5,'lm');
y(pixels_to_remove)=100;
figure;mimg(y,100,100,.4e-3,1.6e-3);colormap(mapgeog)
hold on
h=zeros(10000,1);
...
text(100,35,'V2','FontSize',16)
text(96,22,'V4','FontSize',16)
[n1 x1]=hist(mean(cond5n_dt_bl(roi_contour2,43:53,16),2)-1,-1.5e-3:1.5e-4:3e-3);
[n2 x2]=hist(mean(cond5n_dt_bl(roi_maskin,43:53,16),2)-1,-1.5e-3:1.5e-4:3e-3);
figure;bar(x1,[n1/size(roi_contour2,1);n2/size(roi_maskin,1)]')
xlim([-1e-3 3e-3]);
legend('circle','background')
...
ranksum(mean(cond5n_dt_bl(roi_contour2,43:53,16),2)-1,mean(cond5n_dt_bl(roi_maskin,43:53,16),2)-1)


%1111d cond5
y=mfilt2(mean(cond5n_dt_bl(:,43:53,3),2)-1,100,100,1.5,'lm');
y(pixels_to_remove)=100;
figure;mimg(y,100,100,.8e-3,2.2e-3);colormap(mapgeog)
hold on
h=zeros(10000,1);
h(roi_contour2)=1;
contour(reshape(h,100,100)')
...
text(100,35,'V2','FontSize',16)
text(96,22,'V4','FontSize',16)
[n1 x1]=hist(mean(cond5n_dt_bl(roi_contour2,43:53,3),2)-1,-1.5e-3:1.5e-4:3e-3);
[n2 x2]=hist(mean(cond5n_dt_bl(roi_maskin,43:53,3),2)-1,-1.5e-3:1.5e-4:3e-3);
figure;bar(x1,[n1/size(roi_contour2,1);n2/size(roi_maskin,1)]')
xlim([-0e-3 3e-3]);
legend('circle','background')
...
ranksum(mean(cond5n_dt_bl(roi_contour2,43:53,3),2)-1,mean(cond5n_dt_bl(roi_maskin,43:53,3),2)-1)


%c
circ_c1c=squeeze(mean(cond1n_dt_bl(roi_contour2,43:53,:),2))-1;
bg_c1c=squeeze(mean(cond1n_dt_bl(roi_maskin,43:53,:),2))-1;
circ_c2c=squeeze(mean(cond2n_dt_bl(roi_contour2,43:53,:),2))-1;
bg_c2c=squeeze(mean(cond2n_dt_bl(roi_maskin,43:53,:),2))-1;
circ_c4c=squeeze(mean(cond4n_dt_bl(roi_contour2,43:53,:),2))-1;
bg_c4c=squeeze(mean(cond4n_dt_bl(roi_maskin,43:53,:),2))-1;
circ_c5c=squeeze(mean(cond5n_dt_bl(roi_contour2,43:53,:),2))-1;
bg_c5c=squeeze(mean(cond5n_dt_bl(roi_maskin,43:53,:),2))-1;


%d
circ_c1d=squeeze(mean(cond1n_dt_bl(roi_contour2,43:53,:),2))-1;
bg_c1d=squeeze(mean(cond1n_dt_bl(roi_maskin,43:53,:),2))-1;
circ_c2d=squeeze(mean(cond2n_dt_bl(roi_contour2,43:53,:),2))-1;
bg_c2d=squeeze(mean(cond2n_dt_bl(roi_maskin,43:53,:),2))-1;
circ_c4d=squeeze(mean(cond4n_dt_bl(roi_contour2,43:53,:),2))-1;
bg_c4d=squeeze(mean(cond4n_dt_bl(roi_maskin,43:53,:),2))-1;
circ_c5d=squeeze(mean(cond5n_dt_bl(roi_contour2,43:53,:),2))-1;
bg_c5d=squeeze(mean(cond5n_dt_bl(roi_maskin,43:53,:),2))-1;


%h
circ_c1h=squeeze(mean(cond1n_dt_bl(roi_contour,43:53,:),2))-1;
bg_c1h=squeeze(mean(cond1n_dt_bl(roi_maskin,43:53,:),2))-1;
circ_c2h=squeeze(mean(cond2n_dt_bl(roi_contour,43:53,:),2))-1;
bg_c2h=squeeze(mean(cond2n_dt_bl(roi_maskin,43:53,:),2))-1;
circ_c4h=squeeze(mean(cond4n_dt_bl(roi_contour,43:53,:),2))-1;
bg_c4h=squeeze(mean(cond4n_dt_bl(roi_maskin,43:53,:),2))-1;
circ_c5h=squeeze(mean(cond5n_dt_bl(roi_contour,43:53,:),2))-1;
bg_c5h=squeeze(mean(cond5n_dt_bl(roi_maskin,43:53,:),2))-1;
...
for i=1:size(circ_c2c,2)
    [n1 x1]=hist(circ_c2c(:,i),-.5e-3:.75e-4:2.5e-3);
    [n2 x2]=hist(bg_c2c(:,i),-.5e-3:.75e-4:2.5e-3);
    figure;bar(x1,[n1;n2]')
    legend('circle','background')
end
...
for i=1:size(circ_c4c,2)
    [n1 x1]=hist(circ_c4c(:,i),-.5e-3:.75e-4:2.5e-3);
    [n2 x2]=hist(bg_c4c(:,i),-.5e-3:.75e-4:2.5e-3);
    figure;bar(x1,[n1;n2]')
    legend('circle','background')
end

for i=1:size(circ_c4c,2)
    p=ranksum(circ_c4c(:,i),bg_c4c(:,i));
    if p>0.05
        disp(i)
        disp(p)
    end
...
bg_c2d=squeeze(mean(cond2n_dt_bl(roi_bg_in,43:53,:),2))-1;
circ_c4d=squeeze(mean(cond4n_dt_bl(roi_contour,43:53,:),2))-1;
bg_c4d=squeeze(mean(cond4n_dt_bl(roi_bg_in,43:53,:),2))-1;
...
bg_c2e=squeeze(mean(cond2n_dt_bl(roi_bg_in,43:53,:),2))-1;
circ_c4e=squeeze(mean(cond4n_dt_bl(roi_contour,43:53,:),2))-1;
bg_c4e=squeeze(mean(cond4n_dt_bl(roi_bg_in,43:53,:),2))-1;
...
bg_c2f=squeeze(mean(cond2n_dt_bl(roi_bg_in,43:53,:),2))-1;
circ_c4f=squeeze(mean(cond4n_dt_bl(roi_contour,43:53,:),2))-1;
bg_c4f=squeeze(mean(cond4n_dt_bl(roi_bg_in,43:53,:),2))-1;
