function [spect_trial,spect_ave,baseline_ave,baseline_std]=spectogram_for_LFP_data_EEG_way(data,win,baseline,len)
...
spect=zeros(NFFT/2,len-win-1);             %when returning to the original data use size(data,2)
for h=1:len:size(PSD_EEG_ROI,2)      %when returning to the original data use size(data,2)
    k=k+1;
    %disp(h)
    for j=1:(len-win-1)      %when returning to the original data use size(data,2)
        %w=hamming(win).';
        w=hanning(win).';
...
%spect_ave=(spect_ave./baseline_ave(:,1*ones(1,size(spect_ave,2))))*100-100;
baseline_tr=nanmean(spect_trial(:,baseline,:),2);
spect_trial=10*(log10(spect_trial)-log10(repmat(baseline_tr,[1 size(spect_trial,2) 1])));
