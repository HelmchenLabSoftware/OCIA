...
for i=[4 3] %condition count
    ...
    eval(['b=size(cond',int2str(i),'n_dt_bl,2);'])
        for h=1:a  %trial count 
            ...
            %cd win320
            cd trials
            eval(['power_alpha_cond',int2str(i),'_trial_',int2str(h),'=squeeze(mean(power_cond',int2str(i),'_trial_',int2str(h),'(4:9,:,:),1));'])
            eval(['save power_alpha_cond',int2str(i),'_trial_',int2str(h),' power_alpha_cond',int2str(i),'_trial_',int2str(h),' pixels'])
            cd ..
            %cd ..
