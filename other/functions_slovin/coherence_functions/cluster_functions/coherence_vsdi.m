...
for i=1:6 %condition count
    ...
    for p=1:size(roi_V1,1)
        for h=1:a  %trial count
            ...
            for ppp=1:size(pixels,1) %pixel count
                for j=1:(b-win) 
                    w=hamming(win).';
                    %w=hanning(win).';
                    eval(['data_win=w.*(cond',int2str(i),'n_dt_hb(pixels(ppp),j:j+win-1)-mean(cond',int2str(i),'n_dt_hb(pixels(ppp),j:j+win-1)));'])
                    y=fft(data_win,NFFT);
                    y=y./win;
                    y = y(:,2:NFFT/2+1);
                    %Pxx=2/0.375*abs(y).^2;
        ...
        eval(['save ',s,'coher_V1_cond',int2str(i),'__pix_',int2str(roi_V1(p)),' coher_V1_ave_pix_',int2str(roi_V1(p)),' roi_V1 pixels'])
