function [vNewPos, nScore, nScoreStd] = wt_track_spot(mI_orig, vPos_orig, mConstraint, mFilter, bDebug)
...
switch lower(sMethod)
    case 'center_of_gravity' % Find center of spot by location the center-of-gravity in segmented image
        ...
        % Segment image
        %  'manual' (center-of-gravity of 5% brightest pixels
        [vB, vIndx] = sort(mROI_conv(:));
        vN = round(length(vB) / 100 * 2.5);
        mSegmentedImage = zeros(size(mROI_conv));
        mSegmentedImage(vIndx(end-vN+1:end)) = 1;

        %  'automatic'
        %mSegmentedImage = im2bw(mROI_conv, graythresh(mROI_conv));
        ...
        %subplot(2,2,1)
        %imagesc(mROI_conv); hold on
        %plot(nX,nY,'sr')
        %subplot(2,2,2)
        %hist(mROI_conv(:))
        %subplot(2,2,3);
        %imagesc(mSegmentedImage); hold on % segment/thresholds        
        %plot(nX,nY,'sr')
