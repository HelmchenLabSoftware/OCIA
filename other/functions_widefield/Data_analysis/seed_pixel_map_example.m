...
y(isnan(y))=10000;
figure;imagesc(y,[.9 1]);colormap(mapgeog)
hold on
h=zeros(205*205,1);
h(roi_s1)=1;
...
y(isnan(y))=10000;
figure;imagesc(y,[.7 1]);colormap(mapgeog)
hold on
h=zeros(205*205,1);
h(roi_s1)=1;
contour(fliplr(reshape(h,205,205)'),'k')
...
cd D:\intrinsic\20150511
load('green_ds.mat')
y=fliplr(smoothn(green_ds,[1 1],'Gauss')');
y(isnan(y))=10000;
figure;imagesc(y,[-.1 .9]);colormap(gray)
hold on
h=zeros(205*205,1);
h(roi_s1)=1;
contour(fliplr(reshape(h,205,205)'),'k')
...
figure;bar([nanmean(d(roi_s1)) nanmean(d(roi_m1)) nanmean(d(roi_m2))])
hold on
errorbar([nanmean(d(roi_s1)) nanmean(d(roi_m1)) nanmean(d(roi_m2))],[nanstd(d(roi_s1)) nanstd(d(roi_m1)) nanstd(d(roi_m2))])
