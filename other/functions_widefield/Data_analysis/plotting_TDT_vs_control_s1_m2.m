cd D:\intrinsic\20160104\a\Matt_files
load('cond_100_ave.mat')
load('cond_1200_ave.mat')
load('rois_s1_m2.mat')

x=(1:size(cond_100_ave,3))*0.05-3.05;
d_con1=reshape(cond_100_ave,256*256,size(cond_100_ave,3));
figure;plot(x,smooth(squeeze(nanmean(d_con1(roi_s1,:),1))-1,1,'Gauss'))
hold on
plot(x,smooth(squeeze(nanmean(d_con1(roi_m2,:),1))-1,1,'Gauss'),'k')
%plot(x,smooth(squeeze(nanmean(d(roi_ppc,:),1))-1,1,'Gauss'),'y')
plot(x,zeros(1,size(cond_100_ave,3)),'k')
con1_s1=squeeze(nanmean(d_con1(roi_s1,:),1))-1;
con1_m2=squeeze(nanmean(d_con1(roi_m2,:),1))-1;

cd D:\intrinsic\20150506\mouse_tgg6fl23_5\d\Matt_files
load('cond_100_ave.mat')
load('cond_1200_ave.mat')
load('rois_s1_m2.mat')

x=(1:size(cond_100_ave,3))*0.05-2.7;
d_con2=reshape(cond_100_ave,205*205,size(cond_100_ave,3));
figure;plot(x,smooth(squeeze(nanmean(d_con2(roi_s1,:),1))-1,1,'Gauss'))
hold on
plot(x,smooth(squeeze(nanmean(d_con2(roi_m2,:),1))-1,1,'Gauss'),'k')
%plot(x,smooth(squeeze(nanmean(d(roi_ppc,:),1))-1,1,'Gauss'),'y')
plot(x,zeros(1,size(cond_100_ave,3)),'k')
con2_s1=squeeze(nanmean(d_con2(roi_s1,:),1))-1;
con2_m2=squeeze(nanmean(d_con2(roi_m2,:),1))-1;
