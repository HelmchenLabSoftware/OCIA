...
if isnumeric(vChWhiskers) & exist('vFrames') % Delete fragment of selected whisker
    ...
    if length(find(g_tWT.MovieInfo.SplinePoints(:,:,:,vChWhiskers))) == 0, clear(vFrames); end
end

if isnumeric(vChWhiskers) & ~exist('vFrames') % Delete all frames of selected whisker
    try, g_tWT.MovieInfo.SplinePoints(:,:,:,vChWhiskers) = []; end
    ...
    try
        ...
        %g_tWT.MovieInfo.WhiskerIdentity{vChWhiskers} = '';
        %g_tWT.MovieInfo.WhiskerIdentity = ...
        %    g_tWT.MovieInfo.WhiskerIdentity(~cellfun('isempty', g_tWT.MovieInfo.WhiskerIdentity));
    end
