function H=shadedErrorBar(x,y,errBar,lineProps,transparent,figHand,axeH)
...
%Set default options
defaultProps = { 'LineStyle', '-', 'Color', 'black' };
...
if nargin<7 || isempty(figHand) || isempty(axeH)
    figHand = figure();
    axeH = gca;
end
...
% H.mainLine = plot(axeH, x, y, lineProps{:});
H.mainLine = stairs(axeH, x, y, lineProps{:});
...
holdStatus=ishold(axeH);
if ~holdStatus, hold(axeH,'on'),  end
...
H.patch=patch(xP,yP,1,'facecolor',patchColor,...
              'edgecolor','none',...
              'facealpha',faceAlpha, 'Parent',axeH);
%Make nice edges around the patch. 
H.edge(1)=plot(axeH,x,lE,'-','color',edgeColor);
H.edge(2)=plot(axeH,x,uE,'-','color',edgeColor);
...
if ~holdStatus, hold(axeH,'off'), end
